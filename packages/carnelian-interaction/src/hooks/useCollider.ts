...
export function useCollider<T>(collider: Collider<T>, hitArea: HitArea, priority: number = 0, element?: DiagramElementNode
) {
    const hitTestCallback: HitTestCallback = (point, transform) => {
        const elemPoint = point.matrixTransform(transform);
        return collide(PointCollider(elemPoint), collider, 0);
    }
